...
export function findClosestIntersection(wire1: string, wire2: string): number {
    const segments1 = parseInput(wire1);
    const segments2 = parseInput(wire2);
    const intersections = findIntersections(segments1, segments2);
    return intersections.reduce((shortestDistance, pt) => Math.min(shortestDistance, Math.abs(pt.x) + Math.abs(pt.y)), Number.MAX_SAFE_INTEGER);
}

function getNextPoint({ x, y }: Point, direction: Direction, totalDistance: number): Point {
    switch (direction) {
        case 'U': return { x, y: y - 1, totalDistance };
        case 'D': return { x, y: y + 1, totalDistance };
        case 'L': return { x: x - 1, y, totalDistance };
        case 'R': return { x: x + 1, y, totalDistance };
    }
}

export function findIntersections(wire1: readonly Segment[], wire2: readonly Segment[]): Point[] {
    const intersections: Point[] = [];
    const points: { [id: string]: Point } = {};
    const origin: Point = { x: 0, y: 0, totalDistance: 0 };
    ...
    wire1.forEach(({ direction, distance }) => {
        for (let i = 0; i < distance; i++) {
            currentPoint = getNextPoint(currentPoint, direction, ++totalDistance);
            const index = `${currentPoint.x},${currentPoint.y}`;
            if (!points[index]) {
    ...
    // now check wire2 segments for intersections
    currentPoint = origin;
    totalDistance = 0;
    wire2.forEach(({ direction, distance }) => {
        for (let i = 0; i < distance; i++) {
            ...
            if (points[index]) {
                intersections.push({
                    ...currentPoint,
                    totalDistance: points[index].totalDistance + currentPoint.totalDistance,
                });
            }
        }
    });
...
export function parseInput(input: string): Segment[] {
    return input.split(',').map(s => {
        const [ , direction, distanceString ] = /^([UDLR])(\d+)$/.exec(s)!;
        return {
            direction,
